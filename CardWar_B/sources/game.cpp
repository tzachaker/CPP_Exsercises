...
namespace ariel{
    ...
    for (const auto& sign : signs)
    {
        ...
        {
            deck.emplace_back(i, sign); //new Card object, without creating a temporary Card object
        }
    }
    std::random_device rd;
    std::mt19937 gen(rd());
    std::shuffle(deck.begin(), deck.end(), gen); //random shuffle of the deck vector
    //auto-> הסקת סוג המשתנה
...
//A reference in C++ is a variable that refers to the same memory location without making a copy of the data.
//Using references can help to avoid unnecessary memory and improve program performance.
    void Game::playTurn()
    ...
    void Game::printWiner()
    {
        ...
        {
            cout<<"A tie between " <<player1.getName()<< " to " <<player2.getName()<<endl;
        }
    }
    void Game::printLog()
    {
